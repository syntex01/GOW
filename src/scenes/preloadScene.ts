...
export default class PreloadScene extends Phaser.Scene {
  ...
  private loadFromManifest(manifest: AssetManifest | null | undefined) {
    ...
    const hasImages = !!(manifest.images && manifest.images.length)
    const hasSheets = !!(manifest.spritesheets && manifest.spritesheets.length)
    const hasAudio = !!(manifest.audio && manifest.audio.length)
    const hasAtlases = !!(manifest.atlases && manifest.atlases.length)
    const hasAssets = hasImages || hasSheets || hasAudio || hasAtlases
