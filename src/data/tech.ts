...
export interface TechNode {
   ...
   *
   * Every creed owns a contiguous CHANNEL of rows, sized to its widest ring,
   * with a two-row gutter between creeds and `core` running down the middle as
   * the trunk. Before this the rows were hand-picked one node at a time and
   * the creeds interleaved: carnage held rows 0-2 and 10, blight held 8-11, and
   * the view's tie-breaker then pushed collisions down into whatever slot was
   * free. The result was readable only by tracing each link with a finger.
   *
   ...
   * the neighbour. Carnage's channel is rows 0-4: rows 0-2 are the old spine,
   * row 3 is the harvest ladder (Skull Tithe, Bone Levy) and row 4 is
   * preservation (Salting, the Deep Cold). Everything from row 4 down was
   * shifted one to make room, which is the widen-and-shift rule above being
   * followed rather than a row being borrowed off ordnance.
   */
  row: number
...
export const TECHS: TechNode[] = [
  ...
  },
  {
    ...
    cost: 1500,
    requires: ['bone_harvest'],
    effect: 'What is lying on your half keeps 70% longer before it is worth nothing. The early field goes cold in seconds; this is the first thing that buys you time to walk out and get it.'
  },
  {
    id: 'deep_cold',
    name: 'The Deep Cold',
    ...
    cost: 2400,
    requires: ['salting'],
    effect: 'Another 60% on top. A late carnage half stops being a race against rot and starts being a larder — and an elite carcass will lie there long enough for a gatherer to cross the board for it.'
  },
  {
    id: 'skull_tithe',
...
export const STAT_TECHS: TechNode[] = [
  ...
  stat('foraging', 'Foraging', 'core', 1, 7, 0, 340, ['collapse'], 'income', 1.15, 'Parties sent out between engagements. Gold arrives faster.'),
  stat('quartermaster', 'Quartermaster', 'core', 1, 9, 0, 360, ['collapse'], 'buildSpeed', 1.2, 'Somebody competent is running the queue. Everything is built sooner.'),
  ...
  stat('shieldwall', 'Shieldwall', 'core', 2, 5, 1, 640, ['rations', 'powder_discipline'], 'toughness', 1.16, 'Trained to stand together. Everything you own takes less from every hit.'),
  stat('spoils', 'Spoils', 'core', 2, 8, 1, 600, ['foraging', 'old_rites'], 'bounty', 1.25, 'What you kill is worth more. Gold and experience both.'),
  stat('long_arms', 'Long Arms', 'core', 2, 10, 1, 660, ['quartermaster', 'powder_discipline'], 'unitRange', 1.12, 'Longer barrels, longer hafts. Your soldiers reach further before they are reached.'),
  ...
  stat('deep_stores', 'Deep Stores', 'core', 3, 5, 2, 1100, ['foraging', 'shieldwall'], 'income', 1.2, 'Reserves nobody has found yet. Income again, and it compounds with the last one.'),
  stat('masonry', 'Masonry', 'engineering', 3, 6, 2, 1150, ['salvage', 'shieldwall'], 'baseHp', 1.25, 'The fortress is rebuilt properly. It holds a great deal more punishment.'),
  stat('war_drums', 'War Drums', 'occult', 3, 8, 2, 1050, ['spoils', 'blood_pact'], 'abilityRate', 1.25, 'Something keeps time. Your special ability comes back faster.'),
  stat('honed_edges', 'Honed Edges', 'carnage', 3, 9, 2, 1200, ['forge_work', 'butchery'], 'unitDamage', 1.1, 'Maintained between engagements rather than after them.'),

  // Ring 4 — the expensive middle, where you are already leaning somewhere.
  stat('tempering', 'Tempering', 'core', 4, 9, 2, 1500, ['forge_work'], 'unitHp', 1.2, 'Worked and quenched again. Everything you build survives more.'),
  stat('marksmanship', 'Marksmanship', 'ordnance', 4, 10, 3, 1700, ['long_arms', 'ricochet'], 'unitRange', 1.15, 'Trained to the sight rather than the volume of fire.'),
  stat('levy', 'Levy', 'core', 4, 11, 2, 1450, ['quartermaster', 'deep_stores'], 'buildSpeed', 1.22, 'Conscription. The queue moves whether or not anyone wants it to.'),
  stat('tribute', 'Tribute', 'occult', 4, 12, 3, 1800, ['spoils', 'war_drums'], 'bounty', 1.3, 'Taken from the dead and counted in front of the living.'),
  stat('outriders', 'Outriders', 'core', 4, 13, 3, 1600, ['forced_march'], 'unitSpeed', 1.15, 'Screening elements ahead of the line. Everything moves up faster.'),

  // Ring 5 — heavy, late, and priced like it.
  stat('plate_lines', 'Plate Lines', 'engineering', 5, 20, 3, 2300, ['tempering', 'masonry'], 'toughness', 1.18, 'Standardised plate, produced in quantity for once.'),
  stat('heavy_powder', 'Heavy Powder', 'ordnance', 5, 5, 4, 2600, ['marksmanship', 'shrapnel'], 'unitDamage', 1.12, 'A coarser, angrier mix. Everything you fire hits appreciably harder.'),
  stat('citadel', 'Citadel', 'engineering', 5, 13, 4, 2700, ['masonry'], 'baseHp', 1.3, 'The fortress is now the strongest thing on the field by a distance.'),
  stat('logistics', 'War Logistics', 'core', 5, 14, 4, 2600, ['deep_stores', 'levy'], 'income', 1.25, 'The whole apparatus behind the line finally works.'),
  stat('zeal', 'Zeal', 'occult', 5, 15, 4, 2500, ['war_drums', 'tribute'], 'abilityRate', 1.3, 'They want to use it. Your ability charges faster again.'),

  // Ring 6 — the last stat nodes, alongside the deep behaviours.
  stat('grand_forge', 'Grand Forge', 'core', 6, 15, 4, 3400, ['heavy_powder', 'honed_edges'], 'unitDamage', 1.12, 'Everything your army carries is made in one place now, and made well.'),
  stat('grand_armoury', 'Grand Armoury', 'core', 6, 5, 4, 3400, ['plate_lines', 'tempering'], 'unitHp', 1.22, 'Full kit, issued to everyone, replaced when it fails.'),
  stat('war_economy', 'War Economy', 'core', 6, 13, 4, 3600, ['logistics', 'citadel'], 'income', 1.3, 'Nothing is produced that is not for this. Income one last time.'),
  stat('total_mobilisation', 'Total Mobilisation', 'core', 6, 14, 4, 3600, ['logistics', 'outriders'], 'buildSpeed', 1.28, 'Everyone who can hold something is holding something.')
]
